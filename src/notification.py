...
class NotificationService(
    ...
    def generate_daily_report(
        ...
        # 标题
        report_lines = [
            f"# 📅 {report_date} Stock Intelligence Analysis Report",
            "",
            f"> Analyzed **{len(results)}** stocks | Report generated at: {datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        report_lines.extend([
            ...
            "| Indicator | Value |",
            "|------|------|",
            f"| 🟢 Buy/Increase Position | **{buy_count}** |",
            f"| 🟡 Hold/Observe | **{hold_count}** |",
            f"| 🔴 Reduce/Sell | **{sell_count}** |",
            f"| 📈 Average Bullish Score | **{avg_score:.1f}** |",
            "",
            "---",
            "",
        ])
        
        # Issue #262: summary_only outputs only summary without individual stock details
        if self._report_summary_only:
            ...
            report_lines.extend(["## 📈 Individual Stock Analysis", ""])
            # 逐个股票的详细分析
            for result in sorted_results:
                ...
                if hasattr(result, 'buy_reason') and result.buy_reason:
                    report_lines.extend([
                        f"**💡 Trade Rationale**：{result.buy_reason}",
                        "",
                    ])
    ...
    def generate_dashboard_report(
        ...
        Format: Market Overview + Important Information + Core Conclusion + Data Pivot + Battle Plan
        ...
        # Sort by score (high score first)
        sorted_results = sorted(results, key=lambda x: x.sentiment_score, reverse=True)

        # Statistics - use decision_type field for accurate count
        buy_count = sum(1 for r in results if getattr(r, 'decision_type', '') == 'buy')
        sell_count = sum(1 for r in results if getattr(r, 'decision_type', '') == 'sell')
        hold_count = sum(1 for r in results if getattr(r, 'decision_type', '') in ('hold', ''))
        ...
        report_lines = [
            ...
            "",
            f"> Analyzed **{len(results)}** stocks | 🟢 Buy:{buy_count} 🟡 Observe:{hold_count} 🔴 Sell:{sell_count}",
            "",
        ]
        ...
        # Individual stock decision dashboard (Issue #262: skip details if summary_only)
        if not self._report_summary_only:
            for result in sorted_results:
                signal_text, signal_emoji, signal_tag = self._get_signal_level(result)
                dashboard = result.dashboard if hasattr(result, 'dashboard') and result.dashboard else {}
                
                # Stock name (prioritize dashboard or result name, escape special characters like *ST)
                ...
                # ========== Intelligence & Fundamental Overview (place first) ==========
